# 🔍 Посібник з діагностики помилок автентифікації KSeF

## Проблема: Помилка на строці 118 (_wait_for_authentication)

Якщо ви бачите помилку:
```
API Error checking auth status: {status_code}
```

Це означає, що сервер KSeF повертає HTTP статус **НЕ 200**.

---

## 🛠️ Як діагностувати проблему

### 1. Запустіть тестовий скрипт з детальним логуванням:

```bash
cd /home/user/einvoice_pl/ksef2
python3 test_auth.py
```

Цей скрипт покаже **детальні логи** кожного кроку автентифікації.

---

## 🔎 Можливі причини помилок

### **HTTP 400 (Bad Request) - Помилка валідації**

**Що це означає:**
- Неправильний формат `reference_number`
- Неправильна структура запиту
- Challenge вже застарів

**Що перевірити:**
```python
# У логах знайдіть рядок:
_logger.info(f'Reference number: {self.reference_number}')
```

**Має бути формат:** `20251212-CR-ABC123DEF456-789`

**Рішення:**
- Перевірте, чи правильно отриманий `referenceNumber` з кроку 4
- Можливо, потрібен новий challenge (зробіть повторну автентифікацію)

---

### **HTTP 401 (Unauthorized) - Не авторизовано**

**Що це означає:**
- Тимчасовий `auth_token` неправильний
- Токен застарів (challenge timeout)
- Токен не відповідає reference_number
- **Неправильний заголовок** (має бути `Authorization: Bearer {token}`)

**Що перевірити:**
```python
# У логах знайдіть рядок:
_logger.info(f'Auth token (first 30 chars): {self.auth_token[:30]}...')
```

**Має бути довгий токен** (100+ символів)

**Рішення:**
1. Перевірте, чи правильно отриманий `authenticationToken.token` з кроку 4
2. Переконайтеся, що використовується саме **auth_token**, а не фінальний token
3. Не чекайте занадто довго між кроками 4 і 5 (challenge може застаріти)
4. **ВАЖЛИВО**: Заголовок має бути `Authorization: Bearer {token}`, а НЕ `SessionToken: {token}`

---

### **HTTP 404 (Not Found) - Не знайдено**

**Що це означає:**
- Неправильний URL endpoint
- Reference number не існує

**Що перевірити:**
```python
# У логах знайдіть рядок:
_logger.info(f'URL: {config.api_url}/api/v2/auth/{self.reference_number}')
```

**Має бути:**
- Test: `https://ksef-test.mf.gov.pl/api/v2/auth/{reference_number}`
- Prod: `https://ksef.mf.gov.pl/api/v2/auth/{reference_number}`

**Рішення:**
- Перевірте `config.api_url`
- Перевірте правильність `reference_number`

---

## 📊 Структура правильної відповіді на кроці 4

```json
{
  "authenticationToken": {
    "token": "ДОВГИЙ-ТОКЕН-100+-СИМВОЛІВ...",
    "type": "internal"
  },
  "referenceNumber": "20251212-CR-ABC123DEF456-789",
  "timestamp": "2025-12-12T10:30:00.000Z"
}
```

**Перевірте в логах:**
```
Step 4 - Response from /auth/ksef-token: {...}
```

Якщо структура інша - є проблема на кроці 4 (ініціалізація автентифікації).

---

## 🔧 Покрокова діагностика

### Крок 1: Перевірте Challenge
```python
# У логах має бути:
# API call to: POST /api/v2/auth/challenge
# Response: {"challenge": "...", "timestamp": "..."}
```

### Крок 2: Перевірте шифрування токена
```python
# Має бути:
# Token to encrypt: {kseftoken}|{timestamp_milliseconds}
```

### Крок 3: Перевірте NIP
```python
# У логах буде:
# contextIdentifier: {"type": "NIP", "value": "9462527947"}
```

Переконайтеся, що NIP витягнутий правильно з `config.kseftoken`.

### Крок 4: Перевірте відповідь
```python
# У логах:
# Step 4 - Response from /auth/ksef-token: {full_response}
```

Якщо замість токена є помилка - проблема у шифруванні або в самому KSeF токені.

### Крок 5: Діагностика polling
```python
# У логах:
# Starting authentication polling...
# Reference number: 20251212-CR-...
# Auth token (first 30 chars): ABC123...
# URL: https://ksef-test.mf.gov.pl/api/v2/auth/20251212-CR-...
```

---

## 🆘 Що робити, якщо нічого не допомагає

1. **Перевірте KSeF токен:**
   ```python
   # У config.py:
   kseftoken = "20251209-EC-1B9E01E000-7E37CF99BC-E2|nip-9462527947|..."
   ```
   - Токен має бути актуальним (не застарілим)
   - Формат: `{date}-{type}-{id}-{hash}|nip-{nip}|{signature}`

2. **Перевірте сертифікат:**
   - Переконайтеся, що сертифікат KsefTokenEncryption завантажується
   - У логах має бути: `Вибираємо сертифікат KsefTokenEncryption`

3. **Перевірте мережу:**
   ```bash
   curl -X POST https://ksef-test.mf.gov.pl/api/v2/auth/challenge
   ```
   Має повернути JSON з challenge.

4. **Зверніться до документації API:**
   - https://www.gov.pl/web/kas/ksef-api

---

## 📝 Приклад успішного виводу

```
2025-12-12 10:30:00 - auth - INFO - Step 4 - Response from /auth/ksef-token: {...}
2025-12-12 10:30:00 - auth - INFO - Authentication initiated, reference: 20251212-CR-ABC...
2025-12-12 10:30:00 - auth - INFO - Temporary auth_token: ABC123DEF456...
2025-12-12 10:30:00 - auth - INFO - Starting authentication polling...
2025-12-12 10:30:02 - auth - INFO - Auth status check #1: code=100, desc=Processing
2025-12-12 10:30:04 - auth - INFO - Auth status check #2: code=200, desc=Authorized
2025-12-12 10:30:04 - auth - INFO - Authentication confirmed! ✓
2025-12-12 10:30:04 - auth - INFO - Step 6 - Response from /auth/token/redeem: {...}
2025-12-12 10:30:04 - auth - INFO - Final session token obtained: XYZ789...
```
